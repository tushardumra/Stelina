...
export const AddCategory = () => {
  ...
  const addCategories = async (e) => {
    ...
    if (editmode === true) {
      ...
      try {
        const useApi = await fetch(`http://localhost:9000/api/update-category/${catId}`, {
          method: "Put",
          body: formData,
        });
        if(useApi.ok) {
      ...
      try {
        const useApi = await fetch(
          "http://localhost:9000/api/uploadCategories",
          {
            method: "POST",
            body: formData,
          },
  ...
  const showCategories = async () => {
    try {
      const useApi = await fetch("http://localhost:9000/api/getall");
  ...
  const deleteCateg = async (id) => {
    const useApi = await fetch(`http://localhost:9000/api/deleteCate/${id}`, {
      method: "Delete",
    });
