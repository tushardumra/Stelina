...
export const ProductDetails = () => {
  ...
  const showProdDetails = async () => {
    try {
      const useApi = await fetch(`http://localhost:9000/api/productDetails/${pId}`);
  ...
  const sendToWishlist = async () => {
    try {
      ...
      const useApi = await fetch("http://localhost:9000/api/saveInWishlist", {
        method: "Post",
  ...
  const addInCart = async () => {
    ...
    try {
      ...
      const useApi = await fetch("http://localhost:9000/api/addInShoppingCart", {
        method: "Post",
        body: JSON.stringify(productData),
        headers: {
          "content-type": "application/json;charset=UTF-8"
  ...
  const showProdByCate = async () => {
    try {
      ...
      const useApi = await fetch(`http://localhost:9000/api/productsList/${cId}`);
