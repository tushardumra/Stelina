...
export const AddProduct = () => {
  ...
  const cateInList = async () => {
    const useApi = await fetch("http://localhost:9000/api/getCategories", {
      method: "Get",
    });
    ...
    if (editmode === true) {
      ...
      try {
        const useApi = await fetch(
          `http://localhost:9000/api/update-product/${prodId}`,
          {
            method: "Put",
            body: UpProductInfo,
          },
      ...
      try {
        const useApi = await fetch("http://localhost:9000/api/uploadProducts", {
          method: "Post",
          body: productInfo,
        });
  ...
  const showProducts = async () => {
    try {
      const useApi = await fetch("http://localhost:9000/api/getProducts");
  ...
  const deleteProd = async (id) => {
    const useApi = await fetch(`http://localhost:9000/api/deleteProd/${id}`, {
      method: "Delete",
    });
